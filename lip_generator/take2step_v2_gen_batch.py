...
LEFT_SWING_RANGE = {
    ...
    "y": (-0.02, 0.15),  # Lateral range (meters)
    "yaw": (-0.2, 0.2),
}
RIGHT_SWING_RANGE = {
    "x": (-0.15, 0.15),  # Forward/backward range (meters)
    "y": (-0.15, 0.02),  # Lateral range (meters)
    "yaw": (-0.2, 0.2),
}
...
def solve_stepping_problem(gait, x0, left_target, right_target, target_yaw=0.0, verbose=False):
    ...
    try:
        ...
        if len(x0) != gait.state.nx:
            print(f"Invalid initial state dimension: {len(x0)} vs {gait.state.nx}")
            return None, False

        q0 = x0[:nq]
        if np.any(np.isnan(q0)) or np.any(np.isinf(q0)):
            print("Initial state contains NaN/Inf")
            return None, False
        ...
        if np.any(np.isnan(left_target)) or np.any(np.isnan(right_target)):
            print("Target positions contain NaN")
            return None, False
        ...
        # Wrap solve in try-catch to catch segfaults as exceptions if possible
        try:
            solver.solve(xs, us, MAX_ITERATIONS, False)
        except (RuntimeError, ValueError) as e:
            print(f"Solver failed during optimization: {e}")
            return None, False

        success = solver.stop < SOLVER_THRESHOLD
        return solver, success
    except (RuntimeError, ValueError, ZeroDivisionError) as e:
        print(f"Error in solve_stepping_problem: {e}")
        return None, False
    except Exception as e:
        print(f"Unexpected error in solve_stepping_problem: {type(e).__name__}: {e}")
        return None, False
...
def main():
    ...
    for i, sample in enumerate(grid_samples):
        ...
        # Sanity check: ensure targets are valid before attempting solve
        if np.any(np.isnan(step1['left_target'])) or np.any(np.isnan(step1['right_target'])):
            print("✗ Step 1 targets contain NaN")
            continue
        if np.any(np.isinf(step1['left_target'])) or np.any(np.isinf(step1['right_target'])):
            print("✗ Step 1 targets contain Inf")
            continue
        ...
        # Validate solution contains no NaN or Inf values
        if np.any(np.isnan(q_after_step1)) or np.any(np.isinf(q_after_step1)):
            print("✗ Step 1 solution contains invalid values (NaN/Inf)")
            continue

        # Check joint limits aren't violated badly (±200 rad is very extreme)
        if np.any(np.abs(q_after_step1) > 200):
            print(f"✗ Step 1 solution violates joint bounds: max={np.max(np.abs(q_after_step1)):.2f}")
            continue
        ...
        try:
            ...
            print(f"✗ Forward kinematics failed after step 1: {e}")
            continue

        lf_after_step1 = rdata_step1.oMf[gait.lfId].translation.copy()
        rf_after_step1 = rdata_step1.oMf[gait.rfId].translation.copy()

        # Validate foot positions are reasonable (not NaN or extremely far)
        if np.any(np.isnan(lf_after_step1)) or np.any(np.isnan(rf_after_step1)):
            print("✗ Computed foot positions contain NaN")
            continue
        if np.any(np.abs(lf_after_step1) > 10) or np.any(np.abs(rf_after_step1) > 10):
            print(f"✗ Foot positions unreasonable: LF={lf_after_step1}, RF={rf_after_step1}")
            continue
        ...
        # Validate step 2 solution
        q_after_step2 = solver2.xs[-1][:robot.model.nq]
        if np.any(np.isnan(q_after_step2)) or np.any(np.isinf(q_after_step2)):
            print("✗ Step 2 solution contains invalid values (NaN/Inf)")
            continue
        if np.any(np.abs(q_after_step2) > 200):
            print(f"✗ Step 2 solution violates joint bounds: max={np.max(np.abs(q_after_step2)):.2f}")
            continue
